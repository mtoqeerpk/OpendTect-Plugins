...
bool ProcInst::start( const QString& program, const QStringList& arguments )
{
    bool result = true;
...
bool ProcInst::start( const QString& program, const QStringList& arguments, SeisInfo& si )
{
    bool result = false;
...
// 	Read output array from process stdout 
    if ( waitForReadyRead() )
        ...
        ErrMsg("ProcInst::compute - error reading data from external attribute");
        ErrMsg( readAllStandardError() );
        result =false;
    }
	return result;
...
bool ProcInst::writeSeisInfo( SeisInfo& si )
{
    ...
    qint64 nbytes = sizeof(si);
    qint64 res = QProcess::writeData( (const char*) &si, nbytes );
    if ( res != nbytes ) {
...
bool ProcInst::writeTrcInfo(int z0, int inl, int crl)
{
    ...
    qint64 nbytes = sizeof(ti);
    qint64 res = QProcess::writeData( (const char*) &ti, nbytes );
    if ( res != nbytes ) {
...
{
    bool result = true;
    qint64 nbytes = sizeof(float);
    qint64 nsize = pD->nrSamples * pD->nrTraces * pD->nrInput;
    qint64 res = QProcess::writeData( (const char*) pD->input, nsize );
    if (res != nsize) {
...
bool ProcInst::readData()
{
    ...
    qint64 nbytes = sizeof(float);
    qint64 nsize = pD->nrSamples * pD->nrTraces * pD->nrInput;
    qint64 res = QProcess::readData( (char*) pD->output, nsize );
    if (res != nsize) {
        ErrMsg("ProcInst::readData - error reading data from external attribute");
        ErrMsg(readAllStandardError() );
        return false;
    }
